...
#| message: false
#| warning: false
#| tbl-cap: "Characteristics of the study population by eligibility for each routinely recommended antigen. Columns corresponding to each antigen show the total number of children who were eligible for the respective antigen based on their age at the time of assessment. Categorical variables are presented as number (%), while continuous variables are presented as mean (standard deviation [SD]). Percentages are calculated within the respective antigen-specific population."
...
![Heatwave thresholds and spatial distribution of cumulative heatwave days across Nigeria, 2020–2024. The first panel shows the heatwave thresholds used to classify each DHS cluster as exposed or unexposed to a heatwave. The x-axis represents the 1,380 clusters, ordered by latitude from southern to northern Nigeria, while the y-axis represents the years 2020–2024. The threshold for each cluster and year was defined using the 1-in-4-year return level. The second panel shows the spatial distribution of the DHS clusters, with colours representing the cumulative number of heatwave days experienced by each cluster in each year. The spatial distribution is presented separately for each year. Clusters with zero heatwave days are shown in black and assigned a smaller point size to reduce overplotting and improve visualisation.](images/heatwave-map-05.png)
...
#| message: false
#| warning: false
#| tbl-cap: "The distribution of observations by heatwave exposure status across antigen-specific datasets. Values are presented as number (%), with each column representing the corresponding antigen-specific dataset. Heatwave status indicates whether the child was exposed to a heatwave within the specified exposure window around the vaccine due date."
...
![Heat index classification across DHS clusters, January 2020–December 2024. The x-axis represents individual calendar days from 1 January 2020 to 31 December 2024, while the y-axis represents the 1,380 DHS clusters, arranged by latitude from northern to southern Nigeria. Each tile represents the heat index category assigned to a specific cluster on a specific day.](images/heat-index-class-02.png){#f2}
...
#| message: false
#| warning: false
#| tbl-cap: "Distribution of observations by heat index category across antigen-specific datasets. Values represent the number and percentage of children classified within each heat index category in the respective antigen-specific dataset. Each antigen column represents the children eligible for the corresponding antigen, with heat index exposure assessed within the specified exposure window around the recommended vaccination date."
...
![Kaplan–Meier estimates of cumulative vaccination coverage for the study antigens up to 3 years of age. The figure presents Kaplan–Meier curves for each antigen, incorporating the relevant censoring mechanisms for the corresponding antigen-specific analysis. The x-axis represents age in days, extending to 3 years (1,095 days), while the y-axis represents cumulative vaccination coverage, estimated from the Kaplan–Meier survival probabilities showing the estimated proportion of children who had received each antigen by each age.](images/all-coverage-01.png)
...
#| message: false
#| warning: false
#| tbl-cap: "Unweighted estimates of vaccination coverage and timeliness by antigen. The table presents, for each antigen, the median age at vaccination, vaccination coverage within 28 days of the recommended vaccination date, and vaccination coverage by 3 years of age. Estimates were derived from the Kaplan–Meier analysis."
...
Model coefficients for the rest of the covariates in both models are presented in the appendix section 5.3.

![Exponentiated coefficients from the Weibull accelerated failure time models for heatwave and heat index exposure across the six study antigens. The figure presents exponentiated accelerated failure time (AFT) coefficients as event time ratios (ETRs), with corresponding 95% confidence intervals, for each antigen. Results are shown separately for the two exposure measures. For the heatwave analysis, no heatwave is the reference category. For the heat index analysis, Tolerable is the reference category, with estimates shown for the Caution and Extreme Caution categories. An ETR greater than 1 indicates a longer time to vaccination compared with the reference category, whereas an ETR below 1 indicates a shorter time to vaccination. An ETR of 1 indicates no difference in vaccination timing between the exposure category and its reference group.](images/heat-coef-plot-01.png)
...
The adequacy of the Weibull AFT models was assessed using complementary log-log diagnostic plots of the survey-weighted survival estimates against log(time) as shown in Figure 4.6. In this diagnostic plot, approximately straight and parallel curves provide graphical support for the assumed Weibull form and the AFT structure. The diagnostic plots showed generally parallel patterns across the categorical heatwave exposure groups, although the degree of linearity varied across antigens. The BCG model showed relatively clear evidence of both linearity and parallelism. For the remaining heatwave models, the curves were broadly parallel, although some departures from a clear linear pattern were observed. For the heat index models, the curves generally showed less distinct linear and parallel patterns, particularly for Pentavalent 1–3, Vitamin A, and MCV1.
